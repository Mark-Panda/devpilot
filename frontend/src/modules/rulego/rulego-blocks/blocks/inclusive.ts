...
const blockType = "rulego_inclusive";
const nodeType = "inclusive";
const category = "rulego_condition" as const;
const MAX_INCLUSIVE_CASES = 6;
...
const def: BlockTypeDef = {
  ...
  register(ScratchBlocks, BlocklyF) {
    ...
    blocks[blockType] = {
      ...
      mutationToDom: function (this: Block & { caseCount_?: number; casesJson_?: string }) {
        ...
        xml.setAttribute("casecount", String(Math.max(1, Math.min(MAX_INCLUSIVE_CASES, this.caseCount_ ?? 1))));
      ...
      domToMutation: function (
        ...
        const count = Math.max(1, Math.min(MAX_INCLUSIVE_CASES, parseInt(xml.getAttribute("casecount") || "1", 10)));
      ...
      updateShape_: function (this: Block & { caseCount_?: number; casesJson_?: string }) {
        const n = Math.max(1, Math.min(MAX_INCLUSIVE_CASES, this.caseCount_ ?? 1));
        let casesJson = "";
        try {
          casesJson =
        ...
        let nodeId = "inc1";
        try {
          nodeName = String(this.getFieldValue?.("NODE_NAME") ?? "包容分支");
          nodeId = String(this.getFieldValue?.("NODE_ID") ?? "inc1");
        } catch {}
  ...
  setConfiguration(block, node) {
    ...
    if (typeof domToMutation === "function") {
      const xml = document.createElement("mutation");
      xml.setAttribute("casecount", String(Math.max(1, Math.min(MAX_INCLUSIVE_CASES, casesArr.length))));
      const casesEl = document.createElement("cases");
  ...
  getConnectionBranches(block, helpers) {
    ...
    for (let i = 0; i < Math.min(MAX_INCLUSIVE_CASES, cases.length); i++) {
  ...
  getWalkInputs(block) {
    ...
    const n = Math.min(MAX_INCLUSIVE_CASES, Array.isArray(cases) ? cases.length : 0);
    const inputs: string[] = [];
