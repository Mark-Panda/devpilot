/**
 * LLM 节点：模型、参数、Prompt、技能（与 Blockly llm 块配置对齐）
 */
...
export function LlmNodeRender() {
  ...
  const data = nodeRender.data as LlmConfig;
      ...
      <Row style={{ display: 'flex', gap: 8 }}>
        <div style={{ flex: 1 }}>
          ...
          <InputNumber
            ...
            onNumberChange={(v) =>
              patchParams({ temperature: typeof v === 'number' ? v : 0.6 })
            }
          />
        </div>
        <div style={{ flex: 1 }}>
          ...
          <InputNumber
            ...
            onNumberChange={(v) =>
              patchParams({ maxTokens: typeof v === 'number' ? v : 0 })
            }
          />
        </div>
      </Row>
