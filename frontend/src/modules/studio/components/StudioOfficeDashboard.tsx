    ...
    <div className="studio-office-dashboard grid shrink-0 grid-cols-1 gap-3 border-b-2 border-[var(--so-red)] bg-[var(--so-dashboard)] px-3 py-3 sm:grid-cols-3 sm:gap-4 sm:px-4">
      ...
      <div className="studio-office-card studio-office-card--paper flex min-h-[140px] flex-col border border-[var(--so-red)] p-3">
        <div className="mb-2 flex items-center justify-between gap-2 border-b border-[#c4b49a] pb-1">
          <h3 className="text-xs font-bold text-[var(--so-paper-ink)]">工作室小记</h3>
          {onExpandMemo ? (
            <button
              type="button"
              onClick={onExpandMemo}
              className="shrink-0 border border-[#a16207] bg-[#fef3c7]/80 px-1.5 py-0.5 font-[system-ui] text-[10px] font-medium text-[#713f12] hover:bg-[#fde68a]"
            >
              弹窗查看
            </button>
          ) : null}
        </div>
        <div className="flex-1 overflow-y-auto text-left text-[11px] leading-relaxed text-[var(--so-paper-ink)]">
          {memoLines.length === 0 ? (
      ...
      <div className="studio-office-card flex min-h-[140px] flex-col border border-[var(--so-red)] bg-[var(--so-charcoal)] p-3">
        <h3 className="mb-2 text-xs font-bold text-[#f1f1f1]">协作状态</h3>
        <p className="mb-2 text-[10px] text-[#9ca3af]">根据发送与进度推断；点击可固定展示（再点同一项取消固定）</p>
          ...
          {(Object.keys(MOOD_META) as StudioOfficeMood[]).map((m) => {
            ...
            return (
              <button
                ...
                className={`border px-2 py-2 text-center text-xs font-semibold transition-colors ${
                  active
                    ? 'border-[var(--so-red)] bg-[var(--so-red)] text-white'
                    : 'border-[#444] bg-[#252525] text-[#d1d5db] hover:border-[var(--so-red)] hover:text-white'
                }`}
              >
                {MOOD_META[m].label}
                <span className="mt-0.5 block text-[9px] font-normal opacity-80">{MOOD_META[m].hint}</span>
              </button>
            )
          })}
        </div>
      ...
      <div className="studio-office-card flex min-h-[140px] flex-col border border-[var(--so-red)] bg-[var(--so-charcoal)] p-3">
        <div className="mb-2 flex items-center justify-between gap-2">
          <h3 className="text-xs font-bold text-[#f1f1f1]">成员一览</h3>
          {onExpandMembers ? (
            <button
              type="button"
              onClick={onExpandMembers}
              className="shrink-0 border border-[#555] bg-[#333] px-1.5 py-0.5 font-[system-ui] text-[10px] text-[#e5e5e5] hover:border-[var(--so-red)]"
            >
              弹窗查看
            </button>
          ) : null}
        </div>
        <ul className="max-h-[100px] flex-1 space-y-1.5 overflow-y-auto text-[11px]">
          {members.map((a) => {
            ...
            return (
              ...
              >
                <div>
                  <span className="font-semibold text-[#e5e5e5]">{a.config.name}</span>
                ...
                {!isMain ? (
                  <button
                    ...
                    className="border border-[#555] px-1.5 py-0.5 text-[10px] text-[#ccc] hover:border-[var(--so-red)] hover:text-[var(--so-red)]"
                  >
                    @ 定向
                  </button>
                ) : null}
  ...
  return (
    <div className="studio-office-ticker shrink-0 border-b-2 border-[var(--so-red)] bg-[#0a0a0a] py-1.5">
      <div className="overflow-hidden">
        <div className="studio-office-marquee-inner flex w-max text-[11px] text-[#9ca3af]">
